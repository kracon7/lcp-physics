'''
Gradient direction test using a rod and doing mass estimation
...
def make_world(mass, force):
    ...
    #                     fric_coeff_s=0.9, fric_coeff_b=[0.001, 0.5])
    c1 = Circle([500, 300], 20, mass=mass)
    c2 = Circle([500, 340], 20, mass=mass)
    c3 = Circle([500, 260], 20, mass=mass)
    bodies += [c1, c2, c3]
    joints.append(FixedJoint(c1, c2))
    joints.append(FixedJoint(c1, c3))

    ob = Rect([600, 300], [10, 200], mass=1)
    bodies.append(ob)
    joints.append(TotalConstraint(ob))

    push_force = lambda t: force if t < 0.6 else ExternalForce.ZEROS
...
def main(screen):
    ...
    initial_force = torch.cat([1*torch.rand(1), torch.cos(ang), torch.sin(ang)]).type(DTYPE)
