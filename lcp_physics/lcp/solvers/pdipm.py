...
----------
""")
            shown_lu_warning = True
        return x.lu()


def lu_solve_hack(b, A_LU):
    '''
...
def solve_kkt(Q_LU, d, G, A, S_LU, rx, rs, rz, ry):
    ...
    invQ_rx = lu_solve_hack(rx, Q_LU)  # Q-1 rx
    if neq > 0:
        # A Q-1 rx - ry
        # G Q-1 rx + rs / d - rz
    ...
    dx = lu_solve_hack(g1, Q_LU)  # Q-1 g1 = - Q-1 AT S-1 h
    ds = g2 / d  # g2 / d = (-rs - w) / d
    dz = w[:, neq:]
...
""")
    ...
    G_invQ_GT = torch.bmm(G, torch.lu_solve(G.transpose(1, 2), *Q_LU)) + F
    R = G_invQ_GT.clone()
    S_LU_pivots = torch.IntTensor(range(1, 1 + neq + nineq)).unsqueeze(0) \
        .repeat(nBatch, 1).type_as(Q).int()
    if neq > 0:
        invQ_AT = torch.lu_solve(A.transpose(1, 2), *Q_LU)
        ...
        U_A_invQ_AT_inv = lu_solve_hack(P_A_invQ_AT.bmm(L_A_invQ_AT), LU_A_invQ_AT)
        S_LU_21 = G_invQ_AT.bmm(U_A_invQ_AT_inv)
        T = torch.lu_solve(G_invQ_AT.transpose(1, 2), *LU_A_invQ_AT)
