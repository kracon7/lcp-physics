...
""")
    ...
    G_invQ_GT = torch.bmm(G, lu_solve_hack(G.transpose(1, 2), Q_LU)) + F
    R = G_invQ_GT.clone()
    S_LU_pivots = torch.IntTensor(range(1, 1 + neq + nineq)).unsqueeze(0) \
        .repeat(nBatch, 1).type_as(Q).int()
    if neq > 0:
        invQ_AT = lu_solve_hack(A.transpose(1, 2), Q_LU)
        ...
        T = lu_solve_hack(G_invQ_AT.transpose(1, 2), LU_A_invQ_AT)
